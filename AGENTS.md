...
Browser TTS safe-boundary pauses are completion-gated at execution time with a default 700 ms minimum mental rest: once a safe learner-facing chunk finishes speaking, the next chunk starts only when the learner explicitly submits/skips the chunk. The visible Training editor is learner-paced: only one editable safe chunk is shown, with a `Replay chunk` action that restarts speech from that chunk's exact source-word boundary; submitted chunks remain hidden during practice, future text stays hidden, and unsafe internal TTS slices are merged into the next safe semantic boundary instead of becoming learner-facing wait points. A visible practice chunk is atomic: Dicta suppresses application-scheduled waits between its internal TTS slices, keeps the textarea editable while they play, and does not enable or queue `Enter`/submit until the entire visible chunk has finished speaking. Submitted chunks reappear only in the final submitted review with aggregate and per-chunk stats. Correct typing alone and elapsed timeout do not advance Browser TTS during chunk practice. The final chunk uses `Finish session` and never auto-submits. During a completed/submitted safe pause, Dicta may copy original-script punctuation into completed typed words after a short idle window; cursor/editing conflicts leave punctuation pending for pause, stop, blur, or submit, and punctuation remains non-scoring. The 4000 ms anti-blocking fallback remains for internal non-practice completion gates only, not learner-facing chunk practice. Safe-pause execution defaults are small browser preferences editable from Adaptive Pace Layer Flow Step 5 / Playback loop. Adaptive timeline pause telemetry keeps `requestedPauseMs` as the controller target and records `actualPauseMs` from the resolved completion gate wait, with suppressed internal waits recorded as zero-wait deferred pauses and `pauseGateResolutionReason` set to `completed`, `submitted`, `timeout`, or `no-gate`.

Browser TTS Training does not render a separate Media player card. Before the first playback creates the chunk plan, expose `Play` below the Training textarea; during active chunk practice, move that same action into the chunk action row. Both placements must use the existing playback command and return focus to the active textarea. In active chunk practice, `Enter` uses the same manual submit/finish action as the visible button only after the complete visible chunk audio ends; before then it is consumed without queuing an answer. Tapping and releasing `Shift` without another key replays the current chunk from its exact source-word boundary; using Shift as a modifier does not replay, and `Shift+Enter` still inserts a new line. Each newly activated chunk starts with an empty focused editor and its cursor at position zero; the submitted text remains available only through cumulative state, telemetry, and final review.
