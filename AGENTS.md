...
During active Browser TTS chunk practice, hide the separate Media player card and expose its `Play` action inside the active chunk action row. That chunk-local `Play` action must use the existing playback command and return focus to the active textarea; outside active chunk practice, keep the existing Media player card.
