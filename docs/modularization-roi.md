...
After this characterization, do not recommend extracting `playTtsFromWord` wholesale yet. The next ROI candidate can be a smaller deterministic helper from the `onend` completion/debug-update path if inspection finds explicit inputs and direct tests; otherwise add a mocked SpeechSynthesis characterization harness before moving code. Keep wholesale `resetSession` extraction deferred unless inspection reveals a smaller deterministic helper with a strong test seam.
