...
- `src/components/TrainingView.tsx` and `src/components/training/TrainingChunkInputPanel.tsx`: active chunk presentation hides the separate Media player card and moves its existing `Play` command into the chunk action row. The existing focus handoff returns the learner to the chunk textarea after playback starts; non-chunk Training states retain the Media player card.
