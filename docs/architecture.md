...
- CSS modules under `src/styles/` are grouped by UI/runtime boundary: auth, training shell/session/header/interaction/chunk practice/responsive, workspace shell/responsive, TTS workspace, session status, transcript preview/review, sidebar support/brand/controls, dashboard, admin, leaderboard, adaptive flow/OpenRouter controls styles, bottom metrics, shared controls, app shell, and final responsive breakpoints.
...
- `src/app/useBrowserTtsPracticeChunkRuntime.ts` and `src/app/browserTtsPracticeChunks.ts`: Browser TTS learner-facing chunk state, safe semantic grouping, exact source-word boundaries used by the active `Replay chunk` action, hidden completed-chunk state for cumulative practice-text composition, and optional practice-chunk telemetry ranges that render in the final submitted review.
- `src/components/TrainingView.tsx`, `src/components/training/TrainingInputCard.tsx`, and `TrainingChunkInputPanel.tsx`: Browser TTS Training hides the separate Media player card from the initial state onward. Before the first chunk plan exists, `Play` renders below the textarea; while chunk practice is active, it renders in the chunk action row. Both placements reuse the existing playback command and focus handoff. A visible chunk is an atomic submission boundary: internal TTS slices continue without application-scheduled waits, the textarea remains editable, and `Enter`/submit stay disabled without queuing until all visible-chunk audio completes. After completion, `Enter` delegates to the same manual submit/finish callback as the visible action. A standalone Shift key tap replays the active chunk; Shift used with another key does not replay, and `Shift+Enter` preserves multiline input. The active chunk ID is the textarea identity boundary: a newly activated chunk receives a blank focused editor at cursor position zero, while completed text remains in cumulative runtime state and telemetry.
...
- `src/app/browserTtsNextChunkScheduler.ts` and `src/app/browserTtsChunkCompletionGate.ts`: pure Browser TTS next-chunk scheduling helpers that keep safe-boundary pauses completion-gated while preserving manual submitted behavior and internal non-practice timeout fallback behavior.
- `src/app/completedChunkPunctuation.ts` and `src/app/ttsPracticePunctuation.ts`: Browser TTS practice-text punctuation helpers that copy punctuation from completed source chunks without correcting words or changing scoring.
...
- Adaptive Pace Layer Flow workspace: implementation map for the closed adaptive loop. Phase 1 / Generation owns the live direct OpenRouter Generate Training Session card, including the locally persisted 2-10 minute duration preference, final compact prompt preview, and no-context/context generation variants. Step 5 / Playback loop owns the local Browser TTS safe-pause gate controls for minimum mental rest and max fallback. Training Mode direct generation buttons reuse the Phase 1 duration preference for labels, help text, and OpenRouter job requests. The former benchmark/feedback cockpit dashboard tab no longer exists; adaptive data still feeds Training Mode, OpenRouter generation context behavior, Live Metrics diagnostics, IndexedDB persistence, and Supabase sync internally.
- Training Mode direct generation shows one log row per active or recently settled OpenRouter job, with elapsed time per click and a cancel action for active jobs. Cancel marks the durable job as canceled server-side and removes it from browser polling without creating a generated error session. The job route keeps a persistence buffer before the 300-second Vercel function window, and polling a stale `queued`/`running` durable job marks it failed so the browser removes it from active generation state. Direct requests use `compact-chunks-v1` by default, so OpenRouter returns only title and semantic chunks while Dicta locally builds the full `DictationScript`; legacy full JSON remains accepted for old jobs and fallback responses.
...
- Browser TTS completed session JSON can include optional `practiceChunks` telemetry with source/typed word ranges and resolution values; this is backward-compatible IndexedDB/Supabase JSON payload data and does not require a schema migration.
...
- OpenRouter and other LLM paths generate compact semantic training material only; Dicta owns local `DictationScript` construction, pacing metadata, chunk safety, replay, and playback execution.
...
- Listening Cycle V3 pauses use continuous `pauseMsTarget` as an adaptive intent signal, then Browser TTS resolves whether a pause is safe at the current boundary. Safe learner-facing pauses are chunk-paced with a default 700 ms minimum mental rest: the next chunk can start after that minimum only when the learner manually submits/skips the current chunk. Correct typing and timeout alone do not advance chunk practice. The final chunk requires `Finish session` and never auto-submits. The 4000 ms anti-blocking fallback remains for internal non-practice completion gates. During a completed/submitted safe pause, Dicta can copy punctuation from the original source into completed typed words after a short idle window; cursor/editing conflicts leave that punctuation pending for pause, stop, blur, or submit. Punctuation stays outside points and accuracy calculations. Those defaults are editable in Adaptive Pace Layer Flow Step 5 and persist as small browser preferences. Unsafe or incomplete boundaries are merged into the next safe learner-facing practice chunk instead of creating unnatural wait points. Benchmark timeline points keep `requestedPauseMs` as the controller target, record `actualPauseMs` from the resolved gate wait, and include `pauseGateResolutionReason` (`completed`, `submitted`, `timeout`, or `no-gate`) so requested-vs-actual pause and execution timing reflect what happened.
