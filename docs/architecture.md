...
- Listening Cycle V3 pauses use continuous `pauseMsTarget` as an adaptive intent signal, then Browser TTS resolves whether a pause is safe at the current boundary. Safe learner-facing pauses are completion-gated with a default 700 ms minimum mental rest: the next chunk can start after that minimum as soon as the current chunk is typed with tolerant matching, with a default 4000 ms fallback to avoid blocking. During a completed safe pause, Dicta can copy punctuation from the original source into completed typed words after a short idle window; cursor/editing conflicts leave that punctuation pending for pause, stop, blur, or submit. Punctuation stays outside points and accuracy calculations. Those defaults are editable in Adaptive Pace Layer Flow Step 5 and persist as small browser preferences. Unsafe or incomplete boundaries defer pause pressure instead of creating unnatural wait points. Benchmark timeline points keep `requestedPauseMs` as the controller target, record `actualPauseMs` from the resolved gate wait, and include `pauseGateResolutionReason` (`completed`, `timeout`, or `no-gate`) so requested-vs-actual pause and execution timing reflect what happened.
