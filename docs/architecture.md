...
- `src/app/browserTtsNextChunkScheduler.ts` and `src/app/browserTtsChunkCompletionGate.ts`: pure Browser TTS next-chunk scheduling helpers that keep safe-boundary pauses completion-gated while preserving manual submitted and timeout fallback behavior.
