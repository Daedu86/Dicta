...
Net reduction in `src/App.tsx` from the AdaptiveAdvancedDiagnostics extraction pass:
...
### 7.1 AdaptiveAdvancedDiagnostics

Boundary: adaptive advanced diagnostics shell inside the adaptive workspace.
...
- adaptive advanced diagnostics shell
- advanced decision, architecture, adapters, latest run, live snapshot, and debug counter rendering

Preserved behavior:

- `AdaptiveBenchmarkSection` remained in `App.tsx` immediately after the extracted component.
- adaptive controller logic, benchmark persistence/write paths, selected profile state, localStorage, sync, and session-feedback state stayed in `App.tsx`.
- app-owned adapter-open behavior stayed in `App.tsx`; the new component only invokes the callback.

Recommended follow-up:

- Do not split `AdaptiveBenchmarkSection` yet.
- Revisit only if a concrete maintainability issue appears in the remaining adaptive workspace UI.
